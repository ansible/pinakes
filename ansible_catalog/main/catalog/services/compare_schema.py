...
class CompareSchema:
    """Compare schemas from service plans"""
    ...
    def compare_schema(source_schema, target_schema):
        ...
        # remote and local have same fields in names
            ...
            changed_field_names = CompareSchema._changed_field_names(
    ...
    def _changed_field_names(local_fields, remote_fields, compare_field_names):
        ...
        for name in compare_field_names:
            local_field = CompareSchema._get_field_by_name(local_fields, name)
            remote_field = CompareSchema._get_field_by_name(
                remote_fields, name
            )
            if not CompareSchema._is_same_field(local_field, remote_field):
                changed.append(name)
