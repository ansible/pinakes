...
def test_fetch_service_plans_from_remote_with_enabled_survey():
    ...
    ServicePlanFactory(
        ...
        create_json_schema=json.dumps(schema),
    )
    ...
    assert svc.service_plans[0]["service_offering_id"] == service_offering.id
...
def test_fetch_service_plans_from_remote_with_disabled_survey():
    ...
    assert svc.service_plans[0]["service_offering_id"] == service_offering.id
    assert (
        svc.service_plans[0]["create_json_schema"]["schemaType"]
        == "emptySchema"
    )
