...
@pytest.mark.django_db
def test_service_offering_service_offering_node_list(api_request):
    """Test to list ServiceOfferingNodes by a certain ServiceOffering endpoint"""

    service_offering1 = ServiceOfferingFactory()
    service_offering2 = ServiceOfferingFactory()
    ServiceOfferingNodeFactory(service_offering=service_offering1)
    ServiceOfferingNodeFactory(service_offering=service_offering1)
    service_offering_node = ServiceOfferingNodeFactory(service_offering=service_offering2)

    response = api_request(
        "get", reverse("inventory:offering-nodes-list", args=(service_offering2.id,))
    )
    ...
    assert content["count"] == 1
    assert content["results"][0]["id"] == service_offering_node.id
