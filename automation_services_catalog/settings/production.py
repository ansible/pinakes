""" Production Settings
    defined in /etc/automation_services_catalog/settings.py or in
               /etc/automation_services_catalog/conf.d/settings.py
"""
import sys
import os
import traceback
...
# the environment, defaulting to /etc/automation_services_catalog/conf.d/.
settings_dir = os.environ.get(
    "CATALOG_SETTINGS_DIR", "/etc/automation_services_catalog/conf.d/"
)
settings_files = os.path.join(settings_dir, "*.py")

# Load remaining settings from the global settings file specified in the
# environment, defaulting to /etc/automation_services_catalog/settings.py.
settings_file = os.environ.get(
    "CATALOG_SETTINGS_FILE", "/etc/automation_services_catalog/settings.py"
)

# Attempt to load settings from /etc/automation_services_catalog/settings.py first, followed by
# /etc/automation_services_catalog/conf.d/*.py.
try:
