...
@pytest.mark.django_db
@mock.patch("pinakes.main.catalog.permissions.check_wildcard_permission")
def test_has_object_permission_owner(check_wildcard_permission):
    owner = UserFactory()
    order = OrderFactory(user=owner)
    item = OrderItemFactory(order=order, user=owner)

    request = mock.Mock(user=owner)
    view = mock.Mock(action="update", kwargs={})
...
def test_has_object_permission_non_owner(check_wildcard_permission):
    owner = UserFactory()
    ...
    permission = OrderItemPermission()
    assert permission.has_object_permission(request, view, item) is False
    check_wildcard_permission.assert_called_once_with(
        "catalog:order",
        "update",
        mock.ANY,
