...
def test_order_item_list(api_request):
    ...
    OrderItemFactory()
    response = api_request("get", reverse("catalog:orderitem-list"))
...
def test_order_item_retrieve(api_request):
    ...
    response = api_request(
        "get", reverse("catalog:orderitem-detail", args=(order_item.id,))
    )

    assert response.status_code == 200
    content = json.loads(response.content)
    assert content["id"] == order_item.id
    assert content["username"] == order_item.username
...
def test_order_item_delete(api_request):
    ...
    order_item = OrderItemFactory()
    response = api_request(
        "delete", reverse("catalog:orderitem-detail", args=(order_item.id,))
    )
...
def test_order_item_post(api_request):
    ...
    response = api_request("post", reverse("catalog:orderitem-list"), data)
...
def test_order_item_patch(api_request):
    ...
    response = api_request(
        "patch",
        reverse("catalog:orderitem-detail", args=(order_item.id,)),
        data,
    )
...
def test_order_item_put(api_request):
    ...
    data = {"name": "update"}
    response = api_request(
        "put", reverse("catalog:orderitem-detail", args=(order_item.id,)), data
    )
